"""
Run this ONCE locally to convert model3_best.weights.h5 → model3_best.onnx.

    pip install tf2onnx onnx
    python ML_model/export_to_onnx.py

Commit the generated model3_best.onnx to git — the server then uses onnxruntime
(~80 MB RAM) instead of tensorflow-cpu (~400 MB RAM).
"""
...
WEIGHTS_PATH = str(Path(__file__).parent / 'model3_best.weights.h5')
ONNX_PATH    = str(Path(__file__).parent / 'model3_best.onnx')
...
class BreedingRecommender(keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        overall = (0.40 * diversity +
                   ...
                   0.25 * breed_match)
        return (
            tf.clip_by_value(overall,       0.0, 1.0),
            diversity,
            inbreeding_risk,
            breed_match,
        )
...
# ── Build, warm-up, load weights ──────────────────────────────────────────────

print('Building model...')
backbone, backbone_dim = build_backbone()
model = BreedingRecommender(backbone=backbone, backbone_dim=backbone_dim)

dummy    = tf.zeros((1, 224, 224, 3))
dummy_sp = tf.zeros((1, 1), dtype=tf.int32)
dummy_r  = tf.zeros((1, 1))
model([dummy, dummy_sp, dummy, dummy_sp, dummy_r], training=False)

print(f'Loading weights from {WEIGHTS_PATH}...')
model.load_weights(WEIGHTS_PATH)

# ── Define concrete inference function for ONNX export ────────────────────────

@tf.function(input_signature=[
    tf.TensorSpec(shape=(1, 224, 224, 3), dtype=tf.float32, name='img_a'),
    tf.TensorSpec(shape=(1, 1),           dtype=tf.int32,   name='species_a'),
    tf.TensorSpec(shape=(1, 224, 224, 3), dtype=tf.float32, name='img_b'),
    tf.TensorSpec(shape=(1, 1),           dtype=tf.int32,   name='species_b'),
    tf.TensorSpec(shape=(1, 1),           dtype=tf.float32, name='relatedness'),
])
def predict(img_a, species_a, img_b, species_b, relatedness):
    overall, diversity, inbreeding, breed_match = model(
        [img_a, species_a, img_b, species_b, relatedness], training=False)
    return (
        tf.reshape(overall,      (1,)),
        tf.reshape(diversity,    (1,)),
        tf.reshape(inbreeding,   (1,)),
        tf.reshape(breed_match,  (1,)),
    )
...
try:
    ...
    print('ERROR: tf2onnx not installed.  Run:  pip install tf2onnx onnx', file=sys.stderr)
    sys.exit(1)
...
print(f'\nDone!  Saved to: {ONNX_PATH}')
print('\nNext steps:')
print('  1. git add ML_model/model3_best.onnx')
print('  2. git commit -m "add ONNX model for lightweight inference"')
print('  3. git push  →  Render will rebuild with onnxruntime only')
